...
SERVICE_NAME = pricofy-geocode-it
STACK_SERVICE = PricofyGeocodeItStack
LAMBDA_GEOCODE = pricofy-geocode-it
...
test-geocode: ## Test geocode Lambda (ENV=dev)
	...
	@aws lambda invoke \
		...
		--payload $(shell echo '{"body":"{\"operation\":\"geocode-by-postal\",\"postalCode\":\"00118\"}"}' | base64 -w 0) \
		response.json
