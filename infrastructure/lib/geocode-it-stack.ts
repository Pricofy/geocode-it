...
export class GeocodeItStack extends cdk.Stack {
  ...
  constructor(scope: Construct, id: string, props: GeocodeItStackProps) {
    ...
    const warmupRule = new events.Rule(this, 'WarmupRule', {
      ruleName: `pricofy-geocode-it-warmup-${props.environment}`,
      schedule: events.Schedule.rate(cdk.Duration.minutes(5)),
      description: `Warmup for geocode-it (${props.environment})`,
    });
    ...
    const functionArn = `arn:aws:lambda:${this.region}:${this.account}:function:pricofy-geocode-it`;
