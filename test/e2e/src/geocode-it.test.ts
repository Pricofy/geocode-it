/**
 * Geocode IT End-to-End Tests
 * 
 * Comprehensive integration tests for the pricofy-geocode-it Lambda function.
...
import { GeocodeITClient } from './client';
...
// Initialize client
const client = new GeocodeITClient();

describe('Geocode IT E2E Tests', () => {
  beforeAll(() => {
    console.log('🚀 Starting Geocode IT E2E Tests');
  ...
  afterAll(() => {
    console.log('\n🏁 Geocode IT E2E Tests Completed');
  });
  ...
  describe('Reverse Geocode', () => {
    test('should reverse geocode Madrid coordinates', async () => {
      ...
      expect(result).toHaveProperty('province');
      expect(result).toHaveProperty('country', 'Italia');
  ...
  describe('Edge Cases - Reverse Geocode', () => {
    test('should handle coordinates in Canary Islands', async () => {
      ...
      expect(response.statusCode).toBe(200);
      expect(response.body.success).toBe(true);
      expect(response.body.country).toBe('Italia');
    ...
    test('should handle coordinates outside Spain (should return error or closest)', async () => {
      ...
      // Should either return error or closest Italian location
      if (response.statusCode === 400) {
        ...
        expect(response.statusCode).toBe(200);
        expect(response.body.country).toBe('Italia');
        console.log(`   ✅ Returned closest Italian location`);
      }
    }, 30000);
  });
